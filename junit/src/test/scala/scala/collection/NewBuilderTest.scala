...
class NewBuilderTest {
  ...
  def mapPreservesCollectionType(): Unit = {
    ...
    test[sc.Iterable[_]         ]((sc.Iterable(1):          sc.Iterable[Int]).map(x => x))
    test[sc.Seq[_]              ]((sc.Seq(1):               sc.Iterable[Int]).map(x => x))
    test[sc.LinearSeq[_]        ]((sc.LinearSeq(1):         sc.Iterable[Int]).map(x => x))
    test[sc.LinearSeq[_]        ]((sc.LinearSeq(1):         sc.Seq[Int]     ).map(x => x))
    test[sc.IndexedSeq[_]       ]((sc.IndexedSeq(1):        sc.Iterable[Int]).map(x => x))
    test[sc.IndexedSeq[_]       ]((sc.IndexedSeq(1):        sc.Seq[Int]     ).map(x => x))
    test[sc.Set[_]              ]((sc.Set(1):               sc.Iterable[Int]).map(x => x))
    test[sc.Map[_, _]           ]((sc.Map(1 -> 1):          sc.Map[Int, Int]).map(x => x))

    test[scm.Iterable[_]        ]((scm.Iterable(1):         sc.Iterable[Int]).map(x => x))
    test[scm.Queue[_]           ]((scm.Queue(1):            sc.Iterable[Int]).map(x => x))
    test[scm.Queue[_]           ]((scm.Queue(1):            sc.Seq[Int]     ).map(x => x))
    test[scm.ArraySeq[_]        ]((scm.ArraySeq(1):         sc.Iterable[Int]).map(x => x))
    test[scm.ArraySeq[_]        ]((scm.ArraySeq(1):         sc.Seq[Int]     ).map(x => x))

    test[scm.Buffer[_]          ]((scm.Buffer(1):           sc.Iterable[Int]).map(x => x))
    test[scm.Buffer[_]          ]((scm.Buffer(1):           sc.Seq[Int]     ).map(x => x))
    test[scm.IndexedSeq[_]      ]((scm.IndexedSeq(1):       sc.Iterable[Int]).map(x => x))
    test[scm.IndexedSeq[_]      ]((scm.IndexedSeq(1):       sc.Seq[Int]     ).map(x => x))
    test[scm.ArrayBuffer[_]     ]((scm.ArrayBuffer(1):      sc.Iterable[Int]).map(x => x))
    test[scm.ArrayBuffer[_]     ]((scm.ArrayBuffer(1):      sc.Seq[Int]     ).map(x => x))
    test[scm.ListBuffer[_]      ]((scm.ListBuffer(1):       sc.Iterable[Int]).map(x => x))
    test[scm.ListBuffer[_]      ]((scm.ListBuffer(1):       sc.Seq[Int]     ).map(x => x))
    test[scm.Seq[_]             ]((scm.Seq(1):              sc.Iterable[Int]).map(x => x))
    test[scm.Seq[_]             ]((scm.Seq(1):              sc.Seq[Int]     ).map(x => x))
    test[scm.Set[_]             ]((scm.Set(1):              sc.Iterable[Int]).map(x => x))
    test[scm.Set[_]             ]((scm.Set(1):              sc.Set[Int]     ).map(x => x))
    test[scm.HashSet[_]         ]((scm.HashSet(1):          sc.Iterable[Int]).map(x => x))
    test[scm.HashSet[_]         ]((scm.HashSet(1):          sc.Set[Int]     ).map(x => x))
    test[scm.LinkedHashSet[_]   ]((scm.LinkedHashSet(1):    sc.Iterable[Int]).map(x => x))
    test[scm.LinkedHashSet[_]   ]((scm.LinkedHashSet(1):    sc.Set[Int]     ).map(x => x))

    test[sci.Iterable[_]        ]((sci.Iterable(1):         sc.Iterable[Int]).map(x => x))
    test[sci.LinearSeq[_]       ]((sci.LinearSeq(1):        sc.Iterable[Int]).map(x => x))
    test[sci.LinearSeq[_]       ]((sci.LinearSeq(1):        sc.Seq[Int]     ).map(x => x))
    test[sci.List[_]            ]((sci.List(1):             sc.Iterable[Int]).map(x => x))
    test[sci.List[_]            ]((sci.List(1):             sc.Seq[Int]     ).map(x => x))
    test[sci.LazyList[_]        ]((sci.LazyList(1):         sc.Iterable[Int]).map(x => x))
    test[sci.LazyList[_]        ]((sci.LazyList(1):         sc.Seq[Int]     ).map(x => x))
    test[sci.Queue[_]           ]((sci.Queue(1):            sc.Iterable[Int]).map(x => x))
    test[sci.Queue[_]           ]((sci.Queue(1):            sc.Seq[Int]     ).map(x => x))
    test[sci.IndexedSeq[_]      ]((sci.IndexedSeq(1):       sc.Iterable[Int]).map(x => x))
    test[sci.IndexedSeq[_]      ]((sci.IndexedSeq(1):       sc.Seq[Int]     ).map(x => x))
    test[sci.Vector[_]          ]((sci.Vector(1):           sc.Iterable[Int]).map(x => x))
    test[sci.Vector[_]          ]((sci.Vector(1):           sc.Seq[Int]     ).map(x => x))
    test[sci.Seq[_]             ]((sci.Seq(1):              sc.Iterable[Int]).map(x => x))
    test[sci.Seq[_]             ]((sci.Seq(1):              sc.Seq[Int]     ).map(x => x))
    test[sci.Set[_]             ]((sci.Set(1):              sc.Iterable[Int]).map(x => x))
    test[sci.Set[_]             ]((sci.Set(1):              sc.Set[Int]     ).map(x => x))
    test[sci.ListSet[_]         ]((sci.ListSet(1):          sc.Iterable[Int]).map(x => x))
    test[sci.ListSet[_]         ]((sci.ListSet(1):          sc.Set[Int]     ).map(x => x))
    test[sci.HashSet[_]         ]((sci.HashSet(1):          sc.Iterable[Int]).map(x => x))
    test[sci.HashSet[_]         ]((sci.HashSet(1):          sc.Set[Int]     ).map(x => x))

    test[scp.ParIterable[_]     ]((scp.ParIterable(1):      scp.ParIterable[Int]).map(x => x))
    test[scp.ParSeq[_]          ]((scp.ParSeq(1):           scp.ParIterable[Int]).map(x => x))
    test[scp.ParSeq[_]          ]((scp.ParSeq(1):           scp.ParSeq[Int]     ).map(x => x))
    test[scp.ParSet[_]          ]((scp.ParSet(1):           scp.ParIterable[Int]).map(x => x))
    test[scp.ParSet[_]          ]((scp.ParSet(1):           scp.ParSet[Int]     ).map(x => x))

    test[scpm.ParIterable[_]    ]((scpm.ParIterable(1):     scp.ParIterable[Int]).map(x => x))
    test[scpm.ParSeq[_]         ]((scpm.ParSeq(1):          scp.ParIterable[Int]).map(x => x))
    test[scpm.ParSeq[_]         ]((scpm.ParSeq(1):          scp.ParSeq[Int]     ).map(x => x))
    test[scpm.ParArray[_]       ]((scpm.ParArray(1):        scp.ParIterable[Int]).map(x => x))
    test[scpm.ParArray[_]       ]((scpm.ParArray(1):        scp.ParSeq[Int]     ).map(x => x))
    test[scpm.ParSet[_]         ]((scpm.ParSet(1):          scp.ParIterable[Int]).map(x => x))
    test[scpm.ParSet[_]         ]((scpm.ParSet(1):          scp.ParSet[Int]     ).map(x => x))
    test[scpm.ParHashSet[_]     ]((scpm.ParHashSet(1):      scp.ParIterable[Int]).map(x => x))
    test[scpm.ParHashSet[_]     ]((scpm.ParHashSet(1):      scp.ParSet[Int]     ).map(x => x))

    test[scpi.ParIterable[_]    ]((scpi.ParIterable(1):     scp.ParIterable[Int]).map(x => x))
    test[scpi.ParSeq[_]         ]((scpi.ParSeq(1):          scp.ParIterable[Int]).map(x => x))
    test[scpi.ParSeq[_]         ]((scpi.ParSeq(1):          scp.ParSeq[Int]     ).map(x => x))
    test[scpi.ParVector[_]      ]((scpi.ParVector(1):       scp.ParIterable[Int]).map(x => x))
    test[scpi.ParVector[_]      ]((scpi.ParVector(1):       scp.ParSeq[Int]     ).map(x => x))
    test[scpi.ParSet[_]         ]((scpi.ParSet(1):          scp.ParIterable[Int]).map(x => x))
    test[scpi.ParSet[_]         ]((scpi.ParSet(1):          scp.ParSet[Int]     ).map(x => x))
    test[scpi.ParHashSet[_]     ]((scpi.ParHashSet(1):      scp.ParIterable[Int]).map(x => x))
    test[scpi.ParHashSet[_]     ]((scpi.ParHashSet(1):      scp.ParSet[Int]     ).map(x => x))
